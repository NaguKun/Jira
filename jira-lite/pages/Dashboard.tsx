...
export const Dashboard: React.FC = () => {
  ...
  const { currentUser, issues, projects, isLoading, isAuthenticated, fetchProjects, fetchIssues } = useData();
          ...
          <div className="space-y-4">
            {projects.map(project => (
              <Link to={`/projects/${project.id}`} key={project.id} className="block group">
                <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 group-hover:border-primary transition-colors">
